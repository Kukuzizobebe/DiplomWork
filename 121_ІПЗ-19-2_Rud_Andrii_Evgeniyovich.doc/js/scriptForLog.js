...
Button.addEventListener('click', () => {
    let alertShown = false; // Флаг для отслеживания показа алерта

    for (let elem of Form.elements) {
        if (!(elem.classList.contains('registerbtn')) && !(elem.classList.contains('loginbtn'))) {
            if (elem.value == "") {
                if (languag == '#UA') {
                ...
                if (window.openDatabase && !alertShown) {
                    ...
                    alertShown = true; // Устанавливаем флаг в true, чтобы предотвратить повторный показ алерта
                }
            }
        }
    }
